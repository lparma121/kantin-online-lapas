...
def tampilkan_resi_copy_otomatis(resi_text):
    html_code = f"""
    <div onclick="copyText()" style="cursor: pointer; background-color: #f0f8ff; border: 2px dashed #00AAFF; border-radius: 10px; padding: 15px; text-align: center; margin-bottom: 10px;">
    ...
    function copyText() {{
        ...
        setTimeout(function() {{ document.getElementById("pesan_copy").innerHTML = ""; }}, 2000);
    }}
    </script>
    """
...
def tampilkan_total_copy_otomatis(total_rp, total_raw, kode_unik):
    html_code = f"""
    <div onclick="salinNominal()" style="background-color: #e3f2fd; padding: 15px; border-radius: 10px; border: 2px dashed #00AAFF; text-align: center; cursor: pointer; margin-bottom: 20px;">
...
def buat_struk_image(data_pesanan, list_keranjang, total_bayar, resi):
    ...
    d.line((20, y+10, 480, y+10), fill="gray", width=1); y += 35
    ...
    for item in list_keranjang:
        ...
        d.text((350, y), harga, fill="black"); y += 25
    d.line((20, y+10, 480, y+10), fill="black", width=2); y += 35
    d.text((30, y), "TOTAL TRANSFER", fill="black")
    d.text((350, y), format_rupiah(total_bayar), fill=(200, 0, 0)); y += 50
...
if menu == "🏠 Beranda":
    ...
    st.markdown("<div style='text-align: center; font-size: 18px;'>Selamat datang di era baru pelayanan <b>Lapas Arga Makmur</b>.<br>Belanja Aman, Transparan, dan Tanpa Uang Tunai.</div>", unsafe_allow_html=True)
    st.divider()

    col1, col2, col3 = st.columns(3)
    with col1:
        with st.container(border=True):
            st.markdown("### 💳\n**Sistem Digital**\n\nCegah peredaran uang fisik.")
    with col2:
        with st.container(border=True):
            st.markdown("### 🛡️\n**Aman & Transparan**\n\nMinimalisir penyimpangan.")
    with col3:
        with st.container(border=True):
            st.markdown("### 📝\n**Tercatat Jelas**\n\nKeluarga tenang memantau.")
            ...
            for i, u in enumerate(res_ulasan.data):
                with cols[i]:
                    with st.container(border=True):
                        ...
                        st.caption(f"\"{u['ulasan'] or 'Tanpa komentar'}\"")
    except: st.error("Gagal memuat ulasan.")
...
# =========================================
    ...
    with tab_menu:
        ...
        def popup_add(item):
            ...
            with c1: st.image(item.get('gambar_url', "https://cdn-icons-png.flaticon.com/512/2515/2515263.png"), use_container_width=True)
            with c2:
                ...
                if st.button("Simpan", type="primary", use_container_width=True):
                    ...
                    if jumlah > 0:
                        st.session_state.keranjang.append({"id": item['id'], "nama": item['nama_barang'], "harga": item['harga'], "qty": jumlah})
                    st.rerun()
        ...
        def kartu_produk(item):
            with st.container(border=True):
                ...
                st.markdown(f'<div class="info-box"><div class="nama-produk">{item["nama_barang"]}</div><div class="harga-produk">{format_rupiah(item["harga"])}</div><div class="stok-produk">Stok: {item["stok"]}</div></div>', unsafe_allow_html=True)
                ...
                if st.button(lbl, key=f"btn_{item['id']}", type="primary" if qty_ada > 0 else "secondary", use_container_width=True):
                    popup_add(item)
    ...
    with tab_checkout:
        if st.session_state.nota_sukses:
            ...
            with c_dl: st.download_button("📥 Download Nota", res_data['data'], f"{res_data['resi']}.jpg", "image/jpeg", type="primary", use_container_width=True)
            with c_new: 
            ...
            if not st.session_state.keranjang:
                ...
                total_bayar_final = total_duit + st.session_state.kode_unik
                with st.container(border=True):
                    for x in st.session_state.keranjang: st.write(f"• {x['qty']}x {x['nama']} ({format_rupiah(x['harga']*x['qty'])})")
                    st.divider()
                    tampilkan_total_copy_otomatis(format_rupiah(total_bayar_final), str(total_bayar_final), st.session_state.kode_unik)
                    bayar = st.selectbox("Metode Bayar", ["Transfer Bank", "E-Wallet", "🎫 Voucher"])
                    with st.form("checkout"):
                        pemesan = st.text_input("Nama Pengirim")
                        untuk = st.text_input("Nama WBP + Bin/Binti")
                        wa = st.text_input("WhatsApp")
                        bukti = st.file_uploader("Upload Bukti", type=['jpg','png'])
                        if st.form_submit_button("✅ Kirim Pesanan", type="primary"):
                            if bukti and pemesan and untuk and wa:
                                try:
                                    f_bytes = bukti.getvalue()
                                    url = upload_file_bytes(f_bytes, "bukti_transfer", f"tf_{int(time.time())}.jpg")
                                    if url:
                                        items_str = ", ".join([f"{x['qty']}x {x['nama']}" for x in st.session_state.keranjang])
                                        resi = generate_resi()
                                        data = {
                                            "nama_pemesan": pemesan, "untuk_siapa": untuk, "nomor_wa": wa,
                                            "item_pesanan": items_str, "total_harga": total_bayar_final,
    ...
    if total_duit > 0:
        with st.container(border=True):
            ...
            c_f1, c_f2 = st.columns([1.5, 1], vertical_alignment="center")
            c_f1.markdown(f"**Total: {format_rupiah(total_duit)}**")
            if c_f2.button("🛒 Troli", type="primary", use_container_width=True): show_cart_modal()
...
# =========================================
    ...
    resi_in = st.text_input("Masukkan Kode Resi (4 digit terakhir juga boleh)")
    if st.button("Cek Status") or (resi_in and 'resi_aktif' in st.session_state):
        if resi_in:
            ...
            if res.data:
                ...
                if d.get('foto_penerima'): st.image(d['foto_penerima'], width=300, caption="Bukti Serah Terima")
                
                if d['status'] == "Selesai" and d.get('rating') is None:
                    with st.form("ulasan"):
                        rate = st.selectbox("Rating", [5,4,3,2,1])
