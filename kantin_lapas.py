...
def generate_resi():
    tanggal = datetime.now(timezone.utc).plus(hours=7).strftime("%d%m")
...
def buat_struk_image(data_pesanan, list_keranjang, total_bayar, resi):
    ...
    d.text((30, y), f"PENERIMA  : {data_pesanan['untuk_siapa']}", fill="black"); y+=35
    
    for item in list_keranjang:
        d.text((30, y), f"- {item['nama'][:25]} ({item['qty']}x)", fill="black")
        d.text((350, y), format_rupiah(item['harga']*item['qty']), fill="black")
        y += 25
    
    d.line((20, y+10, 480, y+10), fill="black", width=2)
    d.text((30, y+30), "TOTAL TRANSFER", fill="black")
    d.text((350, y+30), format_rupiah(total_bayar), fill=(200, 0, 0))
...
# =========================================
if menu == "🏠 Beranda":
    st.markdown("<h1 style='text-align:center;'>e-PAS Mart</h1>", unsafe_allow_html=True)
    st.info("Selamat datang di layanan belanja digital Lapas Arga Makmur.")
            ...
            for i, u in enumerate(res_ulasan.data):
                with cols[i]:
                    with st.container(border=True):
                        st.markdown(f"**{u['nama_pemesan']}** {'⭐' * u['rating']}")
                        st.caption(f"\"{u['ulasan'] if u['ulasan'] else '-'}\"")
    except: pass
...
# =========================================
    ...
    # --- DIALOG KERANJANG (FIXED REFRESH) ---
        ...
        if not st.session_state.keranjang:
            ...
            for i, item in enumerate(st.session_state.keranjang):
                with st.container(border=True):
                    ...
                    # Key unik menggunakan ID agar tidak error saat hapus
                    if c3.button("🗑️", key=f"del_item_{item['id']}_{i}"):
                        st.session_state.keranjang.pop(i)
                        st.rerun()
            
            st.divider()
            curr_total = sum(x['harga'] * x['qty'] for x in st.session_state.keranjang)
            st.markdown(f"#### Total: {format_rupiah(curr_total)}")
            
            if st.button("💳 Lanjut Pembayaran", type="primary", use_container_width=True):
                st.toast("Silakan pindah ke Tab Pembayaran")
                time.sleep(0.5)
                st.rerun()
    ...
    with tab_menu:
        ...
        if items:
            ...
            for idx, item in enumerate(items):
                with cols[idx % 2]:
                    with st.container(border=True):
                        st.image(item['gambar_url'] if item['gambar_url'] else "https://via.placeholder.com/150", use_container_width=True)
                        st.markdown(f"**{item['nama_barang']}**")
                        st.markdown(f"<span style='color:#00AAFF; font-weight:bold;'>{format_rupiah(item['harga'])}</span>", unsafe_allow_html=True)
                        st.caption(f"Stok: {item['stok']}")
                        qty_ada = sum(x['qty'] for x in st.session_state.keranjang if x['nama'] == item['nama_barang'])
                        if st.button(f"✅ {qty_ada} di Troli" if qty_ada > 0 else "Beli +", key=f"buy_{item['id']}", use_container_width=True, type="primary" if qty_ada > 0 else "secondary"):
                            popup_add(item)
        else:
    ...
    with tab_checkout:
        if st.session_state.nota_sukses:
            ...
            st.image(res_data['data'], width=300)
            if st.button("🔄 Belanja Lagi"):
                st.session_state.nota_sukses = None
                st.rerun()
        else:
            if not st.session_state.keranjang:
                st.info("Keranjang kosong. Pilih barang dulu.")
            else:
                ...
                with st.form("form_bayar"):
                    ...
                    if st.form_submit_button("✅ Kirim Pesanan", type="primary"):
                        if pemesan and untuk and wa and bukti:
                            try:
                                url = upload_file_bytes(bukti.getvalue(), "bukti_transfer", f"tf_{int(time.time())}.jpg")
                                items_str = ", ".join([f"{x['qty']}x {x['nama']}" for x in st.session_state.keranjang])
                                resi = f"KANTIN-{int(time.time())}"
                                
                                data = {
                                    "nama_pemesan": pemesan, "untuk_siapa": untuk, "nomor_wa": wa,
                                    "item_pesanan": items_str, "total_harga": total_bayar_final,
                                    "bukti_transfer": url, "status": "Menunggu Verifikasi", "no_resi": resi
                                }
                                supabase.table("pesanan").insert(data).execute()
                                
                                # Update Stok
                                for x in st.session_state.keranjang:
                                    curr = supabase.table("barang").select("stok").eq("id", x['id']).execute()
                                    if curr.data:
                                        supabase.table("barang").update({"stok": curr.data[0]['stok'] - x['qty']}).eq("id", x['id']).execute()
    ...
    if total_duit > 0:
        ...
        with st.container(border=True):
            ...
            c1.write(f"**Total: {format_rupiah(total_duit)}**")
            if c2.button("🛒 Lihat Troli", type="primary", use_container_width=True):
                show_cart_modal()
...
# =========================================
    ...
    if st.button("Cek Status"):
        ...
        if res.data:
            ...
            st.write(f"Barang: {d['item_pesanan']}")
            if d.get('foto_penerima'):
                st.image(d['foto_penerima'], caption="Bukti Serah Terima")
        else:
